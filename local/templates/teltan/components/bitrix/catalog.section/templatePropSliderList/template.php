    ...
    <div class="vip-items-slider">
        <?php foreach ($arResult['ITEMS'] as $arItem):?>
            <div class="card product-card item-vip-slider item-vip-slider_full-image"
                 <?php if (!empty($arItem['VIP_COLOR'])):?>style="background-color: <?=$arItem['VIP_COLOR']?>;"<?php endif;?>
            >
                ...
                <?php if (!empty($arItem['PROPERTIES']['LENTA_DATE']['VALUE']) && strtotime($arItem['PROPERTIES']['LENTA_DATE']['VALUE']) > time()) :?>
                    <div class="d-flex marker">
                        <?php if (!empty($arItem['PROPERTIES']['TYPE_TAPE']['VALUE']['NAME'])):?>
                            <div class="d-flex flex-column decor-rec" style="border-color: <?=$arItem['PROPERTIES']['TYPE_TAPE']['VALUE']['COLOR']?>;">
                                <div class="rec-top"></div>
                                <div class="rec-bottom"></div>
                            </div>
                            <div class="text" style="background-color:<?=$arItem['PROPERTIES']['TYPE_TAPE']['VALUE']['COLOR']?>;">
                                <?=$arItem['PROPERTIES']['TYPE_TAPE']['VALUE']['NAME']?>
                            </div>
                        <?php endif;?>
                    </div>
                <?php endif;?>
                <div class="px-2 px-lg-3 content-block">
                    ...
                    <div class="d-flex flex-column text-right">
                        <a href="<?=$arParams['SECTION_PAGE_URL']?>" class="category"><?=$arParams['SECTION_NAME']?></a>
