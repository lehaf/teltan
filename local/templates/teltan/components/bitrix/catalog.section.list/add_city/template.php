...
<div class="form-group row flex-column-reverse flex-lg-row">
                ...
                <ul class="show-city <?= ($arParams['PROPS']['UF_CITY']['VALUE'] != '')? 'selected': '' ?>">
                    <? foreach ($arCity as $id => $citys) {
                        foreach ($citys as $city) {
                            ?>
                            <li class="cityClassSelector"><label for="city"><input data-parent-class="parentClass<?= $id ?>" name="city"
                                                         value="<?= $city['NAME'] ?>"
                                                         type="radio"><?= $city['NAME'] ?></label></li>
                        <? } ?>
                    <? } ?>
                </ul>
            </div>
    ...
    <label for="userSelectSection" class="col col-lg-2 label-name">
        <?= Loc::getMessage('Country'); ?> *
    </label>
</div>

<?=($arParams['PROPS']['UF_REGION']['VALUE'] != '')? "<script>renderCity('parentClass$regionSelectedId')</script>":'' ?>