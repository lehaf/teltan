...
JCSmartFilter.prototype.keyupSection = function(input)
{
	if(!!this.timer)
	{
		clearTimeout(this.timer);
...
JCSmartFilter.prototype.reloadSection = function(input)
{
	// document.location.href = input.value;
	// return;
	// console.log(input.value);

	var bxajaxid = document.getElementsByName('bxajaxid')[0].value;
	var url = BX.util.htmlspecialcharsback(input.value) + '?bxajaxid='+bxajaxid;
	var COMPONENT_CONTAINER_ID = 'comp_' + bxajaxid;
	// console.log(document.getElementsByName('bxajaxid')[0].value);
	...
	BX.ajax.insertToNode(url, COMPONENT_CONTAINER_ID);
};
...
JCSmartFilter.prototype.selectDropDownSectionItem = function(element, controlId)
{
	this.keyupSection(BX(controlId));
