...
$this->addExternalJs(SITE_TEMPLATE_PATH.'/libs/jquery-ui-1.9.2.custom.min.js');
$this->addExternalCss(SITE_TEMPLATE_PATH.'/libs/jquery-ui-1.9.2.custom.min.css');

//pr($arResult['SECTIONS_TREE']);

//pr($arParams['SECTION_ID'])
$SECTION_ID_L1 = $SECTION_ID_L2 = 0;
?>
<div class="smart-filter <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "smart-filter-horizontal"?>">
	<div class="smart-filter-section">
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smart-filter-form filter">
            ...
            // выводит рубрикатор по разделам
			$arCur = [
				'CONTROL_NAME_ALT' => 'SECTION_L1',
				'CONTROL_ID' => 'SECTION_L1',
				'LABEL' => 'Направление',
                'SECTIONS_LIST' => $arResult['SECTIONS_TREE'],
                'CURRENT_SECTION_ID' => 0,
			];

			include '_form-item-section.php';

			$SECTION_ID_L1 = $arCur['CURRENT_SECTION_ID'];

			$arCur = [
				'CONTROL_NAME_ALT' => 'SECTION_L2',
				'CONTROL_ID'       => 'SECTION_L2',
				'LABEL'            => 'Рубрика',
				'SECTIONS_LIST'    => $arResult['SECTIONS_TREE'][$SECTION_ID_L1]['ITEMS'],
			];

			include '_form-item-section.php';

			$SECTION_ID_L2 = $arCur['CURRENT_SECTION_ID'];

			$arCur = [
				'CONTROL_NAME_ALT' => 'SECTION_L3',
				'CONTROL_ID'       => 'SECTION_L3',
				'LABEL'            => 'Категория',
				'SECTIONS_LIST'    => $arResult['SECTIONS_TREE'][$SECTION_ID_L1]['ITEMS'][$SECTION_ID_L2]['ITEMS'],
			];
			include '_form-item-section.php';
            ...
            foreach($arResult["ITEMS"] as $key=>$arItem)
            {
	            ...
	            if ($arItem["CODE"] == 'AUCTION_DATE_START_REQUIRED')
                {
	                $arItem["NAME"] .= ' (от)';
                }
	            ?>
                ...
                <div class="form-item">
                    ...
                    <label class="zag"><span><?=$arItem["NAME"]?></span></label>
                        <?
                        ...
                        <div class="smart-filter-input-container smart-filter-calendar-container" style="display: none;">
                            <?$APPLICATION->IncludeComponent(
                                'bitrix:main.calendar',
                                '',
                                array(
