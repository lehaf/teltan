        ...
        <? foreach ($arResult['ITEMS'] as $arItem) { ?>
            <div class="mb-4 col">
                ...
                if ($arItem['PROPERTIES']['COLOR_DATE']['VALUE'] && strtotime($arItem['PROPERTIES']['COLOR_DATE']['VALUE']) > time())
                    $color = '#FFF5D9';
                ?>
                <div class="card product-card auto-product" style="background-color: <?= $color; ?>">
                    ...
                    <div class="px-2 px-lg-3 content-block">
                        <div class="text-right">
                            ...
                            <? if ($arItem['PROPERTIES']['PROP_PROBEG']['VALUE'] != null) {
                                ?>
                                <p class="mb-2 mileage"><?= number_format($arItem['PROPERTIES']['PROP_PROBEG']['VALUE'], 0, '.', ' '); ?> <?= $arItem['PROPERTIES']['PROP_KM_ML']['VALUE'] ?>
                                    <i class="ml-2 icon-download-speed"></i></p>
                            <? } ?>
                            <? if ($arItem['PROPERTIES']['PROP_PROBEG_Left']['VALUE'] != null) {
                                ?>
                                <p class="mb-2 mileage"><?= number_format($arItem['PROPERTIES']['PROP_PROBEG_Left']['VALUE'], 0, '.', ' '); ?> <?= $arItem['PROPERTIES']['PROP_KM_ML']['VALUE'] ?>
                                    <i class="ml-2 icon-download-speed"></i></p>
                            <? } ?>
