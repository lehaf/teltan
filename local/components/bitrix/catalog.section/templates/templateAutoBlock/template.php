...
?><script>console.log(<?=json_encode($arResult)?>)</script><?php
...
<?foreach($arResult['ITEMS']  as $arItem) {
    ...
    <div class="mb-4 col">
        <div class="card product-card" style="background-color: <?=$color;?>">
            ...
            <div class="px-2 px-lg-3 content-block">
                <div class="text-right">
                    ...
                    <?if($arItem['PROPERTIES']['PROP_PROBEG_Left']['VALUE'] != null){?>
                        <p class="mb-2 mileage"><?=number_format($arItem['PROPERTIES']['PROP_PROBEG_Left']['VALUE'], 0, '.', ' ');?> <?=$arItem['PROPERTIES']['PROP_KM_ML']['VALUE']?><i class="ml-2 icon-download-speed"></i></p>
                    <?}?>
            ...
            <?if(strtotime($arItem['PROPERTIES']['LENTA_DATE']['VALUE']) > time()){?>
