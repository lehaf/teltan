...
<?php if (!empty($arResult['ADS'])):?>
    <div class="row row-cols-2 row-cols-lg-1">
        <?php foreach ($arResult['ADS'] as $key => $item):
            $itemCounter++;
            ...
            <div class="mb-3 col" id="<?=$this->GetEditAreaID($item['ID'])?>">
                <div class="card product-card product-line <?=strtotime($item['VIP_DATE']) > time() ? 'product-line-vip property-vip' : ''?>"
                ...
                >
                    <div class="card-link">
                        ...
                        <div class="px-2 px-lg-3 d-flex justify-content-between like-price">
                            ...
                            <?php endif;?>
                            <?php if (strtotime($item['VIP_DATE']) > time()):?>
    ...
    <?=$arResult['PAGINATION']?>
<?php else:?>
    <div class="empty-ads"><?=Loc::getMessage('UFL_EMPTY_ITEMS')?></div>
<?php endif;?>
