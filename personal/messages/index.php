...
if (isset($_GET['id']) && isset($_GET['au']) && isset($_GET['ref'])) {
    ...
    <?} else {
        ...
        $messages = getMessagesChat($_GET['id'], $userId, $_GET['au']);
        ?>
                    ...
                    <div style="overflow-y: scroll;height: 600px;"
                        ...
                        <div class="chat__message-window">
                            ...
                            foreach ($messages as $message) {
                                ...
                                ?>
                                <div class="d-flex flex-column text-right message<?= ($message['UF_AUTOR_ID'] == $userId) ? ' message-sand' : ''; ?>">
                                    ...
                                    if ($message['UF_AUTOR_ID'] == $userId) {?>
                                        <p class="mb-0 status">
                                            <?= ($message['UF_READ']) ? Loc::getMessage('READ') : Loc::getMessage('SEND'); ?>
                                        </p>
                                    <?} ?>
                                </div>
                            <?} ?>
                        ...
                        <? } else { ?>
                            <div class="d-flex flex-column disableChat">
                                ...
                                <div class="d-flex flex-column align-items-end text-danger disableChat__message">
                                    <? if ($blockedAuthor) { ?>
                                        <p class="mb-2"><?= Loc::getMessage('blockedAuthor') ?></p>
                                        <p class="mb-0"><?= Loc::getMessage('Block2') ?></p>
                                        <?
                                    } ?>
                                    <? if ($blockedUser) { ?>
                                        <p class="mb-2"><?= Loc::getMessage('blockedUser') ?></p>
            ...
            <div class="col-12 col-lg-9 block_chats">
                ...
                <? } else {
                    ...
                    foreach ($chats as $chat) {
                        ...
                        $secUserID = ($chat['ID_AUTOR'] == $userId) ? $chat['ID_SEC_USER'] : $chat['ID_AUTOR'];
                        $secondUser = getUserInfoByID($secUserID);
                        $adTitle = getTitleAD($chat['ID_AD']);
                        $adPreviewPicture = getPreviewPictAD($chat['ID_AD']);
                        foreach ($chats as $message) {
                            if ($message['UF_BLOCK_AUTOR'] && $message['UF_AUTOR_ID'] == $userId) {
                                $blockedAuthor = true;
                                $blocked = true;
                                if ($message['UF_DATE_BLOCK'])
                                    $blockTime = $message['UF_DATE_BLOCK']->format("d.m.Y");
                            ...
                            if ($message['UF_BLOCK_USER'] && $message['UF_ID_USER'] == $userId) {
                                $blockedUser = true;
                                $blocked = true;
                                if ($message['UF_DATE_BLOCK'])
                                    $blockTime = $message['UF_DATE_BLOCK']->format("d.m.Y");
                                ...
                                <div class="col-4 col-md-3 col-xl-2 d-flex justify-content-center">
                                    ...
                                    <? if ($block) {?>
                                        <div class="danger-mesage">
                                            ...
                                            <? if ($blockedAuthor) { ?>
                                                <p class="mb-0 text-danger"><?= Loc::getMessage('blockedAuthor') ?></p>
                                            <?} ?>
                                            <? if ($blockedUser) { ?>
